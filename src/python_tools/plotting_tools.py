...
def plot_reference_frames( frames, args, vectors = [], plots = [], planes = [] ):
	...
	#frame are lists of numpy arrays, so for each of these lists you print them
	for frame in frames:
		'''
		...
		'''
		ax.quiver( zeros, zeros, zeros,						#iniciando na origem
			frame[ 0, : ], frame[ 1, : ], frame[ 2, : ],	#cada frame é uma coordenada, mas plota como colunas 
